...
struct Cli {
    ...
    /// Optionally bind an admin server to this port on localhost (127.0.0.1) for querying
    /// runtime information via a simple line-based text protocol (e.g. with `netcat`).
    #[arg(long)]
        ...
        tokio::select! {
            ...
            result = listener.accept() => {
                ...
                task_master.spawn(async move {
                    tokio::select! {
                        () = cancel.cancelled() => {}
                        () = handle_admin_connection(stream, publishers, endpoint) => {}
                    }
                });
            }
        }
